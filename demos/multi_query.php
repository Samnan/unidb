	...
	if ( !$db->query( $sql ) ) {
		...
		if ($db->num_rows() > 0) {
			while( $row = $db->fetch_row() ) {
				...
				// perform a second query using the same db object. Note the extra argument to the method calls
				// that inform the db object to use a separate 'stack' other than the default query resource
				// this is done so that the first query result is not overwritten by second query
				
				$sql = "SELECT * from test_table where id = " . $row['id'];
				$db->query( $sql, '_second' );
				while( $row2 = $db->fetch_row('_second') ) {
					// use $row2 in anyway you like
				}
			}
		}
	}
