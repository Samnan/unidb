 ...
 */
	...
	// tell the database we only want numeric indices for result set
	// comment the below line and run the script again to see the changes
	$db->set_option('result_type', 'num');

	// generate a simple SQL statement (using db expression for a demo purposes
	...
	if ( !$db->query( $sql ) ) {
		...
		$row = $db->fetch_row();  // $row is printed below in the HTML code
...
<body>
	...
	<p>returned result set looks like this:
